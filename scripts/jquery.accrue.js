...
;(function( $, window, document, undefined ){
    ...
    $.fn.accrue.options = {
        ...
        response_output_div: ".calculations",
        response_basic:
			'<p class="monthly-payment">You will have a monthly Payment of <span>$%payment_amount%</span></p>' +
			'<table class="calculator-table"><tr><td>If you pay monthly,</td><td>You will pay off the Loan Amount in about...</td><td>And you will end up paying an estimated total intrest of...</td></tr><tr><td>Only Minimum Monthly payment</td><td>%num_payments% months</td><td>$%total_interest%</td></tr><tr><td>Minimum payment + $100</td><td>%min_payment_min% months</td><td>$%min_payment_total%</td></tr></table>' + '<br />' +
			'<table class="calculator-table"><tr><td>If you pay monthly,</td><td>You will pay off the Loan Amount in about...</td><td>And you will end up paying an estimated total intrest of...</td></tr><tr><td>$%ten_month%</td><td>10 months</td><td>$%ten_month_total_int%</td></tr><tr><td>$%six_month%</td><td>6 months</td><td>$%six_month_total_int%</td></tr><tr><td>$%three_month%</td><td>3 months</td><td>$%three_month_total_int%</td></tr></table>',
    ...
    var calculateBasic = function( elem, options, output_elem ){
        ...
        var loan_info = $.loanInfo({
            ...
            //custom: get_field(elem, options, "custom")
        });
        ...
        if ( loan_info!==0 ) {
        	...
        	var output_content = options.response_basic
				...
				.replace("%ten_month%", loan_info.ten_month)
        		.replace("%ten_month_total_int%", loan_info.ten_month_total_int)
				.replace("%min_payment_min%", loan_info.min_payment_min)
                .replace("%min_payment_total%", loan_info.min_payment_total)
        		.replace("%six_month%", loan_info.six_month)
        		.replace("%six_month_total_int%", loan_info.six_month_total_int)
        		.replace("%three_month%", loan_info.three_month)
        		.replace("%three_month_total_int%", loan_info.three_month_total_int);
    ...
    $.loanInfo = function( input ) {
        ...
        if ( amount*rate*term>0 ) {
            ...
            return {
                ...
                payment_amount_formatted: monthly.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"),
                num_payments: term,
                total_payments: ( monthly * term ), 
                total_payments_formatted: ( monthly * term ).toFixed(2), 
                total_interest: ( ( monthly * term ) - amount ),
                total_interest_formatted: ((monthly * term) - amount).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"),

                //min_payment_min: Math.log-(1-(amount * rate/(monthly * 12)))/(12 * Math.log(1+(rate/12))),   
                min_payment_min: ((Math.log(-1 / (1 - (amount * rate / 12)) / (monthly + 100)) / Math.log(1 + rate / 12)) / Math.log(10) * -8.025).toFixed(2),
                min_payment_total: ((((Math.log(-1 / (1 - (amount * rate / 12)) / (monthly + 100)) / Math.log(1 + rate / 12)) / Math.log(10) * -8.025)* monthly)-amount).toFixed(2),
                
                ten_month: ((+amount + ((((amount * (Math.pow(1 + monthly_interest, 10)) * monthly_interest) / ((Math.pow(1 + monthly_interest, 10)) - 1)) * 10) - amount)) / 10).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"),
                ten_month_total_int: ((((amount * (Math.pow(1 + monthly_interest, 10)) * monthly_interest) / ((Math.pow(1 + monthly_interest, 10)) - 1)) * 10) - amount).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"),
                six_month: ((+amount + ((((amount * (Math.pow(1 + monthly_interest, 6)) * monthly_interest) / ((Math.pow(1 + monthly_interest, 6)) - 1)) * 6) - amount)) / 6).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"),
            	six_month_total_int: ((((amount * (Math.pow(1 + monthly_interest, 6)) * monthly_interest) / ((Math.pow(1 + monthly_interest, 6)) - 1)) * 6) - amount).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"),
            	three_month: ((+amount + ((((amount * (Math.pow(1 + monthly_interest, 3)) * monthly_interest) / ((Math.pow(1 + monthly_interest, 3)) - 1)) * 3) - amount)) / 3).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"),
            	three_month_total_int: ((((amount * (Math.pow(1 + monthly_interest, 3)) * monthly_interest) / ((Math.pow(1 + monthly_interest, 3)) - 1)) * 3) - amount).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"),

				//min_payment_min: LN(1 / (1 - (LoanBal * IntRate / 12) / (MinMonthDue + 100))) / LN(1 + IntRate / 12)
                //min_payment_min: ((Math.log(1 / (1 - (amount * (rate / 12)) / (monthly + 100)))) / (Math.log(1 + (rate / 12)))).toFixed(2)
            };
        } else {
